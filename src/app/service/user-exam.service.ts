...
export class UserExamService {
    ...
    getAllUserExamByExamId(id: number): Observable<UserExam[]> {
        return this.httpClient.get<UserExam[]>(environment.urlApi + `userExams/exams/${id}`);
    }

    getAllExamsOfUserById(id: number): Observable<Exam[]> {
        return this.httpClient.get<Exam[]>(environment.urlApi + `userExams/users/${id}`);
    }
}
